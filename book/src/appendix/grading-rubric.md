...
This appendix documents the scoring philosophy and criteria used
across the course: the general per-lab rubric, the course-level
grade weights, and the full Week 14 presentation rubric.

---

## 1. Rubric Philosophy

Every full lab (a lab with graded Guided Exercises, not a review or
presentation week) is worth **10 points**, split across that lab's
own deliverables. The exact split varies slightly by lab — a lab with
three guided exercises might split 4/3/3, one with two might split
5/5 — but the total is always 10, and the split is always stated on
that lab's own "Submission and rubric" section.

As a general pattern, expect something close to:

| Criterion | Typical points | What earns full marks |
|---|:---:|---|
| **Completion** | 2–3 | All required guided exercises are attempted and submitted in the correct format |
| **Correctness** | 4–5 | The output, schema, or query matches what the exercise asked for |
| **Style & conventions** | 1–2 | Follows the [SQL Style Guide](sql-style-guide.md): header comment, naming, indentation, named constraints |
| **Reasoning / explanation** | 1–2 | Where a lab asks for a short justification (e.g., "why this normal form", "why this key"), the reasoning is correct and clearly stated |

> If a submitted `.sql` file does not run cleanly against MySQL 8.x
> (see the [SQL Style Guide](sql-style-guide.md)), **Correctness**
> points are not awarded, even if the intended logic looks right on
> paper. Fix every error before submitting.

Review weeks (5, 8, 13, 15) and the presentation week (14) do not use
this 10-point lab rubric; see the course-level weights below and the
Week 14 rubric further down this page.

---

## 2. Course-Level Grade Weights
...
days. No credit after 3 days, unless arranged with the instructor in
advance.

---

## 3. Week 14 Presentation Rubric

Database Systems (511783-001). This is the full grading rubric for
the Week 14 case study presentation, this course's 발표
(presentation) component, 10% of the final grade. The Week 14 slides
show a summary; this section is the complete version, for you to
check before and after presenting.

### 3.1 What You Are Presenting
...
### 3.2 Point Breakdown (100 points total)
...
### 3.5 Common Ways Teams Lose Points
